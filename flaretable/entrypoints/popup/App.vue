...
// --- STATE MANAGEMENT ---
const currentView = ref('choice'); // 'choice' or 'calendar'
const loading = ref(false); // For the "Extract HTML" button
const icsData = ref(''); // Stores the raw base64 string from the backend
const events = ref<any[]>([]); // Stores parsed events for the calendar view
const currentWeekStart = ref(new Date());

// --- COMPUTED PROPERTIES ---
// Controls visibility of "Preview" and "Download" buttons
...
async function extractHtml() {
  ...
  try {
    ...
    // 2. Execute script to get page HTML
    ...
    // 3. Send the FULL HTML to the backend
    const response = await fetch('http://localhost:8787/cf-generate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ html }), // Send the full HTML, not filtered text
    });
...
function showPreview() {
  if (!hasIcsData.value) {
    toast.error('No calendar data to preview.');
    return;
  }
  try {
    // 1. Decode the base64 data
    const icsContent = atob(icsData.value);
    console.log('--- Raw .ics Content from AI ---');
    ...
    console.log(`Found ${vevents.length} events to parse.`);

    // 3. Map to our events array (with added safety checks)
    const parsedEvents = vevents.map((vevent: any) => {
      const event = new ICAL.Event(vevent);
      if (event.startDate && event.endDate) { // Safety check!
        return {
          id: event.uid,
    ...
    }).filter(event => event !== null); // Filter out the nulls

    events.value = parsedEvents as any[];

    if (events.value.length === 0) {
      toast.info('Could not find any valid events in the calendar file.');
    } else {
      toast.success(`Successfully parsed ${events.value.length} events.`);
    }

    // 4. Switch to the calendar view
    currentView.value = 'calendar';
  } catch (err) {
    toast.error('Failed to parse calendar data. Check console for details.');
    console.error('[FlareTable Parsing Error]', err);
  }
}

function downloadIcs() {
  if (!hasIcsData.value) {
    toast.error('No calendar data to download.');
    return;
  }
  try {
...
<style>
/* You can place your existing styles here */
.app-container {
  width: 100%;
  height: 100%;
  background: #f0f2f5;
...
.time-column { font-size: 10px; color: #777; padding-top: 24px; }
